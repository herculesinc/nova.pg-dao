...
export class Store {
    ...
    private readonly validateImmutability   : boolean;
    ...
    // ACCESSORS
    // --------------------------------------------------------------------------------------------
    get<T extends Model>(modelClass: typeof Model, id: string): T | undefined {
        if (!isModelClass(modelClass)) throw new TypeError('Cannot get model: model class is invalid');
